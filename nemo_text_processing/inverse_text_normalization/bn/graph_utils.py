...
from nemo_text_processing.inverse_text_normalization.bn.utils import get_abs_path, load_labels
...
# Bengali digits (০-৯)
NEMO_BN_DIGIT = pynini.union("০", "১", "২", "৩", "৪", "৫", "৬", "৭", "৮", "৯").optimize()
BENGALI_DIGIT = ["০", "১", "২", "৩", "৪", "৫", "৬", "৭", "৮", "৯"]
...
# Bengali words for negative: ঋণ (negative), ঋণাত্মক (negative/minus)
MINUS = pynini.union("ঋণ", "ঋণাত্মক", "বিয়োগ").optimize()


def integer_to_bengali(n: int) -> str:
    """Convert an integer to Bengali digit representation."""
    return ''.join(BENGALI_DIGIT[int(d)] for d in str(n))

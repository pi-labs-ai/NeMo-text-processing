...
class OrdinalFst(GraphFst):
    ...
    Finite state transducer for classifying ordinal
        e.g. তেরোতম -> ordinal { integer: "১৩" }
        e.g. প্রথম -> ordinal { integer: "১" }
    ...
    def __init__(self, cardinal: GraphFst):
        ...
        # Bengali ordinal suffixes: তম (tomo), ম (mo), য় (yo)
        graph = pynini.closure(NEMO_CHAR) + pynini.union(
            graph_digit,
            graph_teens,
            graph_digit_hundred,
            pynini.cross("ভাঁ", "ভাঁ"),
            pynini.cross("ভেঁ", "ভেঁ"),
            pynini.cross("ভে", "ভে"),
        )
        ...
        rule = pynini.cdrewrite(morpho_graph, pynini.closure(NEMO_BN_DIGIT), pynini.union("[EOS]", " "), NEMO_SIGMA)

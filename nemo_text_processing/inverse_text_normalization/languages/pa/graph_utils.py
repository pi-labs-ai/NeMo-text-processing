...
from nemo_text_processing.inverse_text_normalization.pa.utils import get_abs_path, load_labels
...
NEMO_PA_DIGIT = pynini.union("੦", "੧", "੨", "੩", "੪", "੫", "੬", "੭", "੮", "੯").optimize()
GURMUKHI_DIGIT = ["੦", "੧", "੨", "੩", "੪", "੫", "੬", "੭", "੮", "੯"]

# Aliases for backward compatibility
NEMO_HI_DIGIT = NEMO_PA_DIGIT
DEVANAGARI_DIGIT = GURMUKHI_DIGIT
...
# Punjabi words for negative
MINUS = pynini.union("ਰਿਣਾਤਮਕ", "ਨਕਾਰਾਤਮਕ", "ਰਿਣ").optimize()


def integer_to_gurmukhi(n: int) -> str:
    """Convert integer to Gurmukhi (Punjabi) numerals"""
    return ''.join(GURMUKHI_DIGIT[int(d)] for d in str(n))


# Alias for backward compatibility
def integer_to_devanagari(n: int) -> str:
    return integer_to_gurmukhi(n)

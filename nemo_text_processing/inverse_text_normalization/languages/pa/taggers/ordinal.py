...
class OrdinalFst(GraphFst):
    """
    Finite state transducer for classifying ordinal (Punjabi)
        e.g. ਤੇਰ੍ਹਵਾਂ -> ordinal { integer: "੧੩" }
    ...
    def __init__(self, cardinal: GraphFst):
        ...
        # Punjabi ordinal suffixes (masculine and feminine forms)
        # ਵਾਂ (vaan) - masculine
        # ਵੀਂ (veen) - feminine
        # ਵੇਂ (ven) - plural/respectful
        graph = pynini.closure(NEMO_CHAR) + pynini.union(
            graph_digit,
            graph_teens,
            graph_digit_hundred,
            pynini.cross("ਵਾਂ", "ਵਾਂ"),
            pynini.cross("ਵੀਂ", "ਵੀਂ"),
            pynini.cross("ਵੇਂ", "ਵੇਂ"),
            pynini.cross("ਵੇ", "ਵੇ"),
            pynini.cross("ਵੀ", "ਵੀ"),
        )
        ...
        rule = pynini.cdrewrite(morpho_graph, pynini.closure(NEMO_PA_DIGIT), pynini.union("[EOS]", " "), NEMO_SIGMA)

...
class DateFst(GraphFst):
    """
    Finite state transducer for classifying date (Punjabi),
        e.g. ਪੰਜ ਜਨਵਰੀ ਦੋ ਹਜ਼ਾਰ ਬਾਰਾਂ -> date { month: "ਜਨਵਰੀ" day: "੫" year: "੨੦੧੨" preserve_order: true }
        e.g. ਦੋ ਹਜ਼ਾਰ ਬਾਰਾਂ -> date { year: "੨੦੧੨" preserve_order: true }
    Args:
    ...
    def __init__(self, cardinal: GraphFst, ordinal: GraphFst):
        ...
        graph_year = pynutil.add_weight(
            pynini.compose(cardinal.graph_no_exception, pynini.closure(NEMO_PA_DIGIT, 1, 4)), 0.03
        )
        ...
        date_days = pynini.union(*[integer_to_gurmukhi(i) for i in range(1, 32)]).optimize()
        ...
        # Punjabi "ਦੀ/ਦਾ" (of) - possessive marker
        graph_date_exceptions = self.month + delete_space + pynutil.delete("ਦੀ") + delete_space + self.day
        graph_date_exceptions |= self.month + delete_space + pynutil.delete("ਦਾ") + delete_space + self.day

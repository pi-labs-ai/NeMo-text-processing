...
from nemo_text_processing.inverse_text_normalization.ta.taggers.measure import MeasureFst
...
class ClassifyFst(GraphFst):
    """
    Final class that composes all other classification grammars for Tamil (தமிழ்).
    ...
    def __init__(
        ...
        if cache_dir is not None and cache_dir != "None":
            ...
            far_file = os.path.join(cache_dir, f"ta_itn.far")
        if not overwrite_cache and far_file and os.path.exists(far_file):
            self.fst = pynini.Far(far_file, mode="r")["tokenize_and_classify"]
            logging.info(f"ClassifyFst.fst was restored from {far_file}.")
        else:
            logging.info(f"Creating ClassifyFst grammars for Tamil.")
